...
All mirrored sources are explicitly classified. Non-commercial material is physically
separated from commercial-safe/open material. Protected material whose host forbids
...
def download(url: str, dest: Path) -> None:
    ...
    if dest.stat().st_size == 0: raise RuntimeError(f"empty download {url}")
...
def write_inventory(root: Path, sources: list[dict]) -> None:
    ...
    for p in sorted(root.rglob("*")):
        if p.is_file(): files.append({"path":str(p.relative_to(root)),"bytes":p.stat().st_size,"sha256":sha256(p)})
    (root/"INVENTORY.json").write_text(json.dumps({"generated_utc":time.strftime("%Y-%m-%dT%H:%M:%SZ",time.gmtime()),"sources":sources,"files":files},indent=2),encoding="utf-8")
    (root/"SOURCES.md").write_text("# Sources\n\n"+"\n".join(f"- **{s['name']}** — {s['source']} — `{s['usage_class']}` — {s['rights']}" for s in sources)+"\n",encoding="utf-8")
...
def split(src: Path, prefix: Path) -> list[Path]:
    ...
    with src.open("rb") as f:
        ...
        while True:
            b=f.read(PART)
            if not b: break
            p=Path(f"{prefix}.part-{i:03d}"); p.parent.mkdir(parents=True,exist_ok=True); p.write_bytes(b); out.append(p); i+=1
    return out


def transport(root: Path, categories: list[str]) -> None:
    t=root/"transport"; t.mkdir(); parts=[]; originals=[]
    for name in categories:
        src=root/"payload"/name; tar=root/f"{name}.tar"
        subprocess.run(["tar","-cf",str(tar),"-C",str(src),"."],check=True)
        originals.append(f"{sha256(tar)}  {tar.name}"); parts.extend(split(tar,t/name)); tar.unlink()
    (t/"ORIGINAL_SHA256SUMS").write_text("\n".join(originals)+"\n",encoding="utf-8")
    (t/"SHA256SUMS").write_text("\n".join(f"{sha256(p)}  {p.name}" for p in parts)+"\n",encoding="utf-8")
    (t/"REASSEMBLY.md").write_text("# Batch 003\n\nConcatenate each category's `.part-NNN` files in lexical order, verify the recreated TAR against `ORIGINAL_SHA256SUMS`, then extract. `learning_noncommercial` must stay outside commercial indexes/pipelines.\n",encoding="utf-8")
...
def build(root: Path) -> None:
    learning_open=root/"payload"/"learning_open"
    learning_nc=root/"payload"/"learning_noncommercial"
    rendering=root/"payload"/"rendering_vfx"
    procedural=root/"payload"/"procedural_ai_motion"
    catalog=root/"payload"/"catalog_only"
    for d in [learning_open,learning_nc,rendering,procedural,catalog]: d.mkdir(parents=True,exist_ok=True)
    losrc=[]; lnsrc=[]; rsrc=[]; psrc=[]; csrc=[]

    clone("KhronosGroup/Vulkan-Tutorial",learning_open/"Khronos_Vulkan_Tutorial")
    losrc.append({"name":"Khronos Vulkan Tutorial","source":"https://github.com/KhronosGroup/Vulkan-Tutorial","usage_class":"REUSABLE_PERMISSIVE","rights":"Repository content is CC BY-SA 4.0 unless otherwise stated; original code listings include CC0; newer tutorials include Apache-2.0/MIT/BSD components. File-level license remains authoritative."})

    download("https://learnopengl.com/book/learnopengl_book_bw.pdf",learning_nc/"LearnOpenGL"/"learnopengl_book_bw.pdf")
    lnsrc.append({"name":"LearnOpenGL free PDF","source":"https://learnopengl.com/","usage_class":"NONCOMMERCIAL_DERIVATIVE","rights":"Official free PDF is CC BY-NC 3.0; site code samples currently CC BY-NC 4.0 and images/videos CC BY 4.0. Physically isolated from commercial-safe learning material."})
    ...
    (catalog/"GAME_DEV_REFERENCE_CATALOG.md").write_text("""# Catalog-only high-value sources\n\nThese are intentionally NOT mirrored because redistribution/commercial-use terms do not support our reusable material bank.\n\n- **Game AI Pro series** — https://www.gameaipro.com/ — chapters are free to download from the official host, but authors/CRC Press retain copyright and prohibit redistributing/rehosting them.\n- **GPU Gems 1–3** — https://developer.nvidia.com/gpugems — free to read online; book text remains copyrighted and source/CD items have separate terms.\n- **Ubisoft LaFAN1** — https://github.com/ubisoft/ubisoft-laforge-animation-dataset — useful BVH mocap, but CC BY-NC-ND 4.0; keep out of commercial derivative/training use.\n- **AMASS** — https://amass.is.tue.mpg.de/ — registration/source-dataset restrictions; use only under current terms.\n- **Mixamo** — Adobe service/assets; do not scrape or redistribute as an open asset bank.\n- **The Art of Game Design / Game Engine Architecture / Real-Time Rendering / AI for Games** — commercial standard references; buy/borrow lawfully rather than mirroring unauthorized copies.\n""",encoding="utf-8")
    ...
    write_inventory(learning_open,losrc); write_inventory(learning_nc,lnsrc); write_inventory(rendering,rsrc); write_inventory(procedural,psrc); write_inventory(catalog,csrc)
    transport(root,["learning_open","learning_noncommercial","rendering_vfx","procedural_ai_motion","catalog_only"])

if __name__=="__main__":
    ap=argparse.ArgumentParser(); ap.add_argument("--out",default="material-bank-batch-003"); a=ap.parse_args()
    root=Path(a.out).resolve(); shutil.rmtree(root,ignore_errors=True); root.mkdir(parents=True); build(root); print("Batch 003 complete",root)
